...
from backend.tasks import daily_financial_data_update, daily_news_update
...
def create_app():
    ...
    scheduler.add_job(daily_financial_data_update, args=[app], trigger='cron', hour=6, minute=0, day_of_week='mon-fri')
    scheduler.add_job(daily_news_update, args=[app], trigger='cron', hour=6, minute=0, day_of_week='mon-fri')
    scheduler.start()
